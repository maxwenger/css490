...
Our search operation will only allow ``and'' operations between search terms.
``Or'' operations must be done using multiple calls.
...
\includegraphics[scale=0.5]{images/master-schema.png}
\pagebreak
...
{
    "success": true,
    "seqNum": <int> // current seq num in the database.
}
...
\pagebreak
\subsubsection{removeSearchableAuction}
\label{ref:usa}
Removes an auction from the search database that corresponds with the given
auction key.
...
\begin{center}
    \begin{tabular}{| p{7cm} | l |}
        ...
        \textbf{Scenario} & \textbf{Response} \\
        \hline
        Successfully removed an auction &
        \begin{lstlisting}[language=tablejson,firstnumber=1]
{
    "success": true
}
...
\subsection{Master DB Schema Changes}
The sequence number field has been added to the auction table to count and
uniquely identify the order and number of changes made to the auction table.
...
the auction. Sequence number is set to zero.

Create auction will then receive a response from the search service. If the
